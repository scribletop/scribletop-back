...
var _ = Describe("Email Sender", func() {
	...
	Context("Sending mails", func() {
		...
		It("sends a mail to mailtrap", func() {
			err := es.SendEmail("foo#1231 <thomas.ruiz@neoxia.com>", "bar", "new-user", struct{ Link string }{
		...
		It("fails to build with incorrect bindings", func () {
			err := es.SendEmail("foo#1231 <thomas.ruiz@neoxia.com>", "bar", "new-user", struct{}{})
			Expect(err).To(HaveOccurred())
		})

		It("fails to build with nil bindings", func () {
			err := es.SendEmail("foo#1231 <thomas.ruiz@neoxia.com>", "bar", "new-user", nil)
			Expect(err).To(HaveOccurred())
		})

		It("fails with unknown mail", func () {
			err := es.SendEmail("foo#1231 <thomas.ruiz@neoxia.com>", "bar", "___unknown___", struct{}{})
			Expect(err).To(HaveOccurred())
		})
	})
})
